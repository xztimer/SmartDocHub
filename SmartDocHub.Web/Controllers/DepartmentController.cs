...
public class DepartmentController(SmartDocHubDbContext dbContext, IMapper mapper) : ControllerBase
{
    ...
    public async Task<IActionResult> GetList([FromQuery] string? key)
    {
        ...
        {
            var matched = await dbContext.Departments
                .Include(d => d.Parent) 
                .Where(d => !d.IsDeleted && (d.DeptName.Contains(key) || (d.Code != null && d.Code.Contains(key))))
                .ToListAsync();
            ...
            foreach (var item in matched)
            {
                ...
                {
                    if (!resultSet.Add(cur)) break;
                    cur = cur.Parent; 
                }
            }

            return Ok(resultSet.OrderBy(d => d.ParentId).ThenBy(d => d.Sort).ToList());
        }

        var list = await dbContext.Departments
            .Where(d => !d.IsDeleted)
